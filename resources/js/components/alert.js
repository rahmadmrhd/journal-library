...
function alert(id, type, messages, closeable, timeout) {
  ...
  const element = `
    ...
    ${Array.isArray(messages) ?
      `<ul class="mt-1.5 list-inside list-disc">
        ${messages.forEach(message => `<li>${message}</li>`)}
        </ul>`:
      messages}
    </div>
    ${closeable ?
      `<button type="button" x-init="setTimeout(() => show = false, ${timeout ?? 3000})"
...
window.showAlert = (id, type, messages, closeable, timeout) => {
  const alertGroup = document.querySelector('#alert-group');
  alertGroup.appendChild(alert(id, type, messages, closeable, timeout));
}