...
<x-app-layout>
  ...
  <div class="border-b border-gray-200 bg-white shadow-md dark:border-gray-700 dark:bg-gray-800 sm:rounded-lg">
    <div class="flex w-full flex-col items-start justify-between p-4">
      <div class="mb-3">
        <h1 class="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">papers Management</h1>
      </div>
      {{-- Button Control --}}
      <div class="flex w-full flex-col justify-between gap-2 dark:divide-gray-700 md:flex-row md:items-center">
        <div class="mb-4 hidden items-center sm:mb-0 sm:flex">
          <form class="w-full sm:pr-3" action="#" method="GET">
            <label for="papers-search" class="sr-only">Search</label>
            <div class="relative sm:w-full md:w-72 xl:w-96">
              ...
              @endif
              <input type="search" name="search" id="papers-search" value="{{ request('search') }}"
        ...
        <div class="flex w-full items-end gap-2 sm:w-auto">
          <button id="add-paper-btn" x-data=""
            x-on:click.prevent="$dispatch('open-modal', 'add-paper-modal')" class="button primary">
      ...
      {{-- @include('pages.papers.form') --}}

      {{-- @include('pages.papers.delete') --}}

      <x-table minHeight="500px" class="mt-6" :columns="[
          [
              'label' => 'Name',
              'name' => 'name',
              'isSortable' => true,
        ...
        @foreach ($papers as $paper)
          @php
            $paper->readonly = $paper->roles()->pluck('slug')->contains('admin');
          @endphp
          <tr class="border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600">
            ...
            <th scope="row">
              <div class="flex items-center truncate px-6 py-2 text-gray-900 dark:text-white">
                <div class="w-10">
                  @isset($paper->image)
                    <img class="h-10 w-10 rounded-full object-cover" src="{{ asset('storage/' . $paper->image) }}"
                      alt="Profile-{{ $paper->nama }}">
                  @else
                ...
                <div class="ps-3">
                  <div class="w-24 truncate text-base font-semibold lg:w-auto">
                    {{ $paper->getFullName() . ($paper->preferred_name ? ' (' . $paper->preferred_name . ')' : '') }}
                  </div>
                  <div class="font-normal text-gray-500">{{ $paper->papername }}</div>
                </div>
              </div>
            </th>
            <td class="w-[1%] px-6 py-2 text-center">{{ $paper->email }}</td>
            <td class="w-[1%] px-6 py-2 text-center">{{ $paper->institution }}</td>
            <td class="w-[1%] px-6 py-2 text-center">
              <ul>
                @foreach ($paper->roles()->get() as $role)
            ...
            @if (!$paper->readonly)
              <td class="w-[100px] space-x-1 whitespace-nowrap p-4">
                <button title="More" id="more-btn-{{ $paper->id }}" {{ $paper->readonly ? 'disabled' : '' }}
                  data-dropdown-toggle="more-dropdown-{{ $paper->id }}" class="button secondary !p-2"
                  type="button">
                ...
                <!-- Dropdown menu -->
                <div id="more-dropdown-{{ $paper->id }}"
                  class="z-10 hidden w-44 divide-y divide-gray-100 rounded-lg bg-white shadow dark:bg-gray-700">
                  <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                    aria-labelledby="more-btn-{{ $paper->id }}">
                    <li>
                      <button type="button" x-data x-on:click="showUpdatepaper({{ $paper->id }}, event, $dispatch)"
                        class="flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                        Edit
                      </button>
                    </li>
                    <li>
                      <button id="reset-password-btn-{{ $paper->id }}"
                        data-dropdown-toggle="reset-password-dropdown-{{ $paper->id }}"
                      ...
                      <div id="reset-password-dropdown-{{ $paper->id }}"
                        class="z-10 hidden w-44 divide-y divide-gray-100 rounded-lg bg-white shadow dark:bg-gray-700">
                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                          aria-labelledby="reset-password-btn-{{ $paper->id }}">
                          @if ($paper->email_verifies_at)
                            <li>
                              <a href="#"
                          ...
                          <li>
                            <button type="button" x-data=""
                              x-on:click.prevent="showConfirmDeleteModal({{ $paper->id }}, event, $dispatch)"
                              class="block w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                    ...
                    <li class="border-t border-gray-100 dark:border-gray-600">
                      <button type="button" x-data x-on:click="deletepaper({{ $paper->id }}, event, $dispatch)"
                        {{ $paper->readonly ? 'disabled' : '' }}
      ...
      <x-modal name="modal-confirm-reset-password">
        <div id="confirm-reset-password" class="max-w-2xl p-4 text-center md:p-5" x-data x-init="$watch('show', value => {
          ...
          <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
            {{ __("Are you sure to reset this paper's password?") }}
          </h3>
