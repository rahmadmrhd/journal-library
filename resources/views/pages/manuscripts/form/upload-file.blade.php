...
<script>
  window.isReview = @js($manuscript->isReview ?? false);
</script>
<form id="manuscript-form" class="" method="POST"
  ...
  <div id="dropbox" x-data="{ show: true, hover: false }" x-on:drop="hover = false;dropboxOndrop($event, $dispatch);"
    class="min-h-72 {{ $manuscript->isReview ?? false ? '' : 'card' }} group relative flex w-full items-center justify-center">
        ...
        <tr id="row-example"
          ...
          <th class="max-w-48 px-2 py-2">
            <div class="flex w-full items-center gap-x-4">
              ...
              <a id="file-name"
                class="flex-1 cursor-pointer text-gray-800 underline-offset-1 hover:!text-blue-500 hover:underline dark:text-gray-200 dark:hover:!text-blue-400"></a>
          ...
          <th class="min-w-64 w-[10%] px-2 py-2">
            ...
            <x-text-input class="!mb-0 !p-1 !text-xs !font-light" type="select" name="file_type" autofocus
              :disabled="isset($manuscript->isReview)">
      ...
      <ul id="list-file" class="w-full lg:hidden">
        <li id="row-example"
          ...
          <div class="flex w-full items-start gap-x-4 font-bold">
            ...
            <a id="file-name"
              class="flex-grow cursor-pointer text-sm text-gray-800 underline-offset-1 hover:!text-blue-500 hover:underline dark:text-gray-200 dark:hover:!text-blue-400"></a>
            <input type="hidden" name="filesId[]" id="id" disabled>

            @if (!isset($manuscript->isReview))
              <div class="w-12 px-2 text-center">
                <button type="button" id="delete-btn" class="button error hidden !p-2">
                  <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
                      d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z"
          ...
          <x-text-input class="!mb-0 !mt-2 !p-1 !text-xs !font-light" label="File Type" type="select" required
            name="file_type" autofocus :disabled="isset($manuscript->isReview)">
            <option value="" disabled selected>-- Select File Type --</option>
          </x-text-input>
        </li>
      </ul>
