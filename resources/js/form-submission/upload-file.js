...
window.registerFileManuscript = (
  ...
  function getAllFile() {
    const filesEL = [
      ...tableFile.querySelectorAll('[role="file"] input[name="filesId[]"]'),
    ].map((el) => el.value);
    return filesEL;
  }
  ...
  function attachFile(parent, file, file_types, isList) {
    ...
    if (delBtn)
      delBtn.onclick = () => dropboxFile.deleteFile(parent, $dispatch);
    ...
    parent.querySelector("#file-ext").closest("div").style.backgroundColor =
      listTypes[file.mime_type];
    ...
    if (!isList) {
      ...
      selectFileType.onchange = (e) => {
        const selectFileTypeList = dropboxFile.rowFiles[
          inputId.value
        ].rowList.querySelector('select[name="file_type"]');
      ...
      selectFileType.onchange = (e) => {
        const selectFileTypeTable = dropboxFile.rowFiles[
          inputId.value
        ].rowTable.querySelector('select[name="file_type"]');
  ...
  if (files) {
    ...
    if (!isDraft) window.dataForm = window.form?.serialize();
  }
  function updateData(fileId, file_type) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        selectFileTypeTable.querySelector('select[name="file_type"]').value =
          response.file.file_type_id;
        selectFileTypeTable.querySelector(
          'input[name="file_type_before"]',
        ).value = response.file.file_type_id;
        selectFileTypeList.querySelector(
          'input[name="file_type_before"]',
        ).value = response.file.file_type_id;
      ...
      error: function (error) {
        ...
        selectFileTypeTable.querySelector('select[name="file_type"]').value =
          selectFileTypeTable.querySelector(
            'select[name="file_type_before"]',
          ).value;
        selectFileTypeList.querySelector('select[name="file_type"]').value =
          selectFileTypeTable.querySelector(
            'select[name="file_type_before"]',
          ).value;
        showAlert("error", {
  ...
  if (!readOnly) {
        ...
        if (this.readyState == XMLHttpRequest.DONE) {
          try {
            ...
            if (!response.success) {
              rowTable.remove();
              rowList.remove();
              const index = [...inputFile.files].findIndex(
    ...
    dropboxFile.deleteFile = (parent, $dispatch) => {
      ...
      const inputId = parent.querySelector('input[name="filesId[]"]');
      const files = dropboxFile.rowFiles[inputId.value];
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            files.rowTable.remove();
            files.rowList.remove();
            delete dropboxFile.rowFiles[inputId.value];
            showAlert("success", {
        ...
        error: function (error) {
          setProgress(files.rowTable, false);
          setProgress(files.rowList, false);
          showAlert("error", {
