<form id="manuscript-form"
  action="{{ route('manuscripts.storeAuthors', ['subGate' => $manuscript->subGate->slug ?? $subGate->slug, 'manuscript' => $manuscript->id]) }}"
  method="POST" class="flex flex-col gap-y-3" x-data="{
      ...
      search: '',
  }" x-init="$watch('search', val => searchAuthors(@js($subGate->toArray()), authors.map(x => x.id), val, (result) => listAuthor = result));"
    ...
    <div class="relative h-full bg-white shadow dark:bg-gray-800 sm:max-h-full sm:rounded-lg">
      <form id="funder-form" action="" method="POST" x-data="{
        ...
        <div class="mt-0 space-y-6 p-6">
          <form action="{{ route('profile.update', $subGate->slug, absolute: false) }}" method="POST"
            class="space-y-6">
